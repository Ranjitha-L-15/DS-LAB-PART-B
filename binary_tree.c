...
struct node* maketree(){ struct node *p ; int info ;
printf("Enter the info or -1 for null\n"); scanf("%d", &info) ;
if(info == -1) return NULL ;

p = (struct node *)malloc(sizeof(struct node)) ; p->data = info ;
printf("Enter the left child of %d\n",info ); p->left = maketree() ;
printf("Enter the right child of %d\n",info ); p->right = maketree() ;
...
int main(){
struct node *root ; root = maketree();
